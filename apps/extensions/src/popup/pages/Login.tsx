...
export default function Login({ onLoginSuccess }: Props) {
  ...
  async function handleLogin() {
    ...
    if (res.success) {
      ...
      // Detect "user not found" type errors → suggest register
      ...
      if (
        ...
        setError(
          'Account not found. If you\'re new, click "Create account" below.'
        );
      } else {
        setError(msg);
      }
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        {/* Google login — opens web app */}
        <button style={s.googleBtn} onClick={openGoogleLogin}>
          <svg
            width="15"
            height="15"
            viewBox="0 0 24 24"
        ...
        <div style={s.divider}>
          <div style={s.divLine} />
          <span style={s.divText}>new to VaultX?</span>
          <div style={s.divLine} />
        </div>

        <button style={s.registerBtn} onClick={openRegister}>
          Create account →
        </button>
...
const s: Record<string, React.CSSProperties> = {
  ...
  divText: { fontSize: 10, color: '#475569', whiteSpace: 'nowrap' },
  errorBox: {
    padding: '8px 12px',
    borderRadius: 7,
    background: '#450a0a',
